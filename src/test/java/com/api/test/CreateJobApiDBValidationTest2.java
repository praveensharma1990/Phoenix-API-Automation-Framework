...
public class CreateJobApiDBValidationTest2 {
	...
	CreateJobResponseModel createJobResponseModel = given().spec(requestSpecWithAuth(UserRole.FD, payload)).when().post("/job/create").then().log()
				.all().statusCode(200).body(matchesJsonSchemaInClasspath("schemaValidator/createJobApiSchema.json"))
				.body("message", Matchers.equalTo("Job created successfully. ")).extract().as(CreateJobResponseModel.class);
